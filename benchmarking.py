...
class CNF:
  ...
  def initialise_matrix_randomly(self, seed=0):
    ...
    def sample_noreplace(l, k):
      ...
      result=np.zeros((l,k),dtype=int)

      for i in range(l):
        # row = np.random.choice(np.arange(1, k + 1), size=num_cols, replace=False)
        result[i,:] = np.random.choice(np.arange(1,self. N+1), size=k, replace=False)

      return result

    np.random.seed(seed)
    variables_matrix = sample_noreplace(self.L, self.K)
...
class Heuristic:
  ...
  def benchmark(self, N, L, K, resume_from=0,num_iters=100):
  # inits random cnf with seed i \in 0, 1, ..., num_iters-1
  # calls dpll_recursive for each cnf and returns [(result, time_taken)]

    # randomf = open("random_time1.log",'a')
    # randomf.write(f"ratio: {round(L/N,2)}\n")
    
    for i in range(resume_from,num_iters):
        print(f"{i} {L/N}")
        self.split_cnt=0
        process = mp.Process(target=one_run, args=(self,i,N,L,K,"my_heuristic_time_175.log"))
        process.start()
        ...
        if process.is_alive():
            ...
            print("Function completed within 5 minutes")


def one_run(heuristic,seed, N,L,K, fname=""):
    cnf = CNF(N, L, K)
    cnf.initialise_matrix_randomly(seed)
    start_time = time.time()
    print(f"started")
    res = heuristic.solve_dpll(cnf)
    ...
    with open(fname,'a') as f:
        if res is None:
            f.write(f"{N} {round(L/N,2)} {seed} {0} {total_time} {heuristic.split_cnt}\n")

        else:
            f.write(f"{N} {round(L/N,2)} {seed} {1} {total_time} {heuristic.split_cnt}\n")
...
if __name__ == '__main__':
    ...
    # randomth = RandomHeuristic()
    two_clause = TwoClauseHeuristic()
    ratios = np.arange(30, 62, 2)/10
    # ratios = np.arange(38, 62, 2)/10
    # ratios = [4.0]
    # print(ratios)
    N=175

    # with open("random_time.log",'a') as randomf:
    #     randomf.write(f"N ratio SAT time splitting_cnt\n")
    for r in ratios:
      print(r)
      two_clause.benchmark(N, int(N*r), 3, resume_from=0,num_iters=100)
